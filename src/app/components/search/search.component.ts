...
export class SearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.appService.apps$.subscribe({
      next: (appList) => {
        // Create the app list with the original apps
        let apps = appList.map(app => ({
          ...app,
          safeIcon: this.sanitizer.bypassSecurityTrustHtml(app.icon)
        }));

        // Add the settings as a regular app item
        ...
        // Add the settings app to the list
        this.sanitizedAppList = [settingsApp, ...apps];
